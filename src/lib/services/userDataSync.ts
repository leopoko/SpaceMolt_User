...
class UserDataSyncService {
  ...
  private collectLocalData(): UserSyncData {
    ...
    try {
      const mm = localStorage.getItem(SYNC_KEYS.marketMemos);
      if (mm) data.marketMemos = JSON.parse(mm);
    } catch { /* ignore */ }
    try {
      const sm = localStorage.getItem(SYNC_KEYS.systemMemos);
      if (sm) data.systemMemos = JSON.parse(sm);
    } catch { /* ignore */ }
    try {
      const stm = localStorage.getItem(SYNC_KEYS.storageMemos);
      if (stm) data.storageMemos = JSON.parse(stm);
    } catch { /* ignore */ }
    try {
      const bk = localStorage.getItem(SYNC_KEYS.bookmarks);
      if (bk) data.bookmarks = JSON.parse(bk);
    } catch { /* ignore */ }
    try {
      const cf = localStorage.getItem(SYNC_KEYS.craftingFavorites);
      if (cf) data.craftingFavorites = JSON.parse(cf);
    } catch { /* ignore */ }
    try {
      const ct = localStorage.getItem(SYNC_KEYS.contacts);
      if (ct) data.contacts = JSON.parse(ct);
    } catch { /* ignore */ }
    try {
      const lp = localStorage.getItem(SYNC_KEYS.loops);
      if (lp) data.loops = JSON.parse(lp);
    } catch { /* ignore */ }
    try {
      const ex = localStorage.getItem(SYNC_KEYS.explorer);
  ...
  private mergeServerData(server: UserSyncData) {
    ...
    if (server.marketMemos) {
      const localRaw = localStorage.getItem(SYNC_KEYS.marketMemos);
      ...
      localStorage.setItem(SYNC_KEYS.marketMemos, JSON.stringify(merged));
    ...
    if (server.systemMemos) {
      const localRaw = localStorage.getItem(SYNC_KEYS.systemMemos);
      const local: Record<string, { savedAt?: string }> = localRaw ? JSON.parse(localRaw) : {};
      const merged = { ...local };
      ...
      localStorage.setItem(SYNC_KEYS.systemMemos, JSON.stringify(merged));
    ...
    if (server.storageMemos) {
      const localRaw = localStorage.getItem(SYNC_KEYS.storageMemos);
      const local: Record<string, { savedAt?: string }> = localRaw ? JSON.parse(localRaw) : {};
      const merged = { ...local };
      ...
      localStorage.setItem(SYNC_KEYS.storageMemos, JSON.stringify(merged));
      storageMemoStore.memos = merged as typeof storageMemoStore.memos;
    }

    // Bookmarks: union of both sets
    if (server.bookmarks) {
      const localRaw = localStorage.getItem(SYNC_KEYS.bookmarks);
      const local: string[] = localRaw ? JSON.parse(localRaw) : [];
      const merged = [...new Set([...local, ...server.bookmarks])];
      localStorage.setItem(SYNC_KEYS.bookmarks, JSON.stringify(merged));
      bookmarkStore.ids = new Set(merged);
    }

    // Crafting favorites: union
    if (server.craftingFavorites) {
      const localRaw = localStorage.getItem(SYNC_KEYS.craftingFavorites);
      const local: string[] = localRaw ? JSON.parse(localRaw) : [];
      const merged = [...new Set([...local, ...server.craftingFavorites])];
      localStorage.setItem(SYNC_KEYS.craftingFavorites, JSON.stringify(merged));
    }

    // Contacts: server wins (more complete source)
    if (server.contacts) {
      const localRaw = localStorage.getItem(SYNC_KEYS.contacts);
      ...
      localStorage.setItem(SYNC_KEYS.contacts, JSON.stringify(merged));
    }

    // Loops: merge by ID
    if (server.loops && Array.isArray(server.loops)) {
      const localRaw = localStorage.getItem(SYNC_KEYS.loops);
      ...
      localStorage.setItem(SYNC_KEYS.loops, JSON.stringify(merged));
    }

    // Explorer: server wins if local is empty
    if (server.explorer) {
      const localRaw = localStorage.getItem(SYNC_KEYS.explorer);
      const local = localRaw ? JSON.parse(localRaw) : null;
      if (!local?.homeBaseSystemId && server.explorer.homeBaseSystemId) {
        localStorage.setItem(SYNC_KEYS.explorer, JSON.stringify(server.explorer));
