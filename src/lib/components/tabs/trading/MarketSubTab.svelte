<script lang="ts">
  ...
  // Auto-refresh market when tab is active and docked
  $effect(() => {
  ...
  function getInventory(itemId: string): number {
    let total = 0;
    for (const c of shipStore.cargo) {
      if (c.item_id === itemId) total += c.quantity;
    }
    for (const s of baseStore.items) {
      if (s.item_id === itemId) total += s.quantity;
    }
    return total;
  }
  ...
  function createOrder() {
    ...
    // For sell orders, cap quantity at available inventory
    if (orderType === 'sell') {
      const available = getInventory(itemId);
  ...
  function quickSell(item: MarketItem, order: MarketOrderEntry) {
    ...
    const available = getInventory(item.item_id);
    const qty = available > 0 && order.quantity > available ? available : order.quantity;
    actionQueueStore.enqueue(
      `Sell ${qty}x ${item.item_name} @₡${order.price_each}`,
      () => ws.createSellOrder(item.item_id, qty, order.price_each)
    );
  }
...
<div class="trading-layout">
  ...
  {#if playerStore.isDocked}
    <!-- DOCKED: Live Market View -->
    {#if !marketStore.data}
      <Card class="space-card">
        <Content>
          <p class="empty-hint">Press <strong>Refresh</strong> to load market data.</p>
        </Content>
      </Card>
    {:else}
      <!-- Market Items Table -->
